...
const ConnectButton: React.FC<ButtonProps> = ({ children, className }) => {
  ...
  useEffect(() => {
    if (account) {
      ...
      if (
        window?.localStorage?.getItem(connectorLocalStorageKey) === ConnectorNames.Vtimes &&
        visionWeb
      ) {
  ...
  return (
    <div className={`relative z-10 ${className}`}>
      <ButtonC
        className={`bg-[#181717] font-bold text-[#ffffff] text-[12px] md:text-[18px] ${className}`}
        onClick={() => {
          if (!account) {
      ...
      >
        {account ? address : t('CONNECT')}
      </ButtonC>
      {showOther && (
        <div
          onClick={() => setShowOther(false)}
          className="fixed w-full h-full z-40 left-0 top-0 opacity-0"
        >
          1
        </div>
      )}
